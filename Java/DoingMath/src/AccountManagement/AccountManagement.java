...
public class AccountManagement {
	...
	public static void main(String[] args) {
		/*
		 * Sau khi khởi tạo các đối tượng tiến hành quản lý account với các hành động 1.
		 * Thêm mới Account 2. Hiển thị danh sách account 3. Xóa Account theo số tài
		 * khoản 4. Sắp xếp theo số tài khoản 5. Tìm kiếm theo số tài khoản 6. Thoát
		 * chương trình
		 */
		System.out.println("------------------------ * * * ---------------------------");
	...
	public static void themMoiAccThuong() {
		...
		/*
		 * C2 String ngaySinhString = sc.nextLine(); LocalDate ngayThangNamSinh =
		 * LocalDate.parse(ngaySinhString); DatetimeFormatter formatter =
		 * DatetimeFormatter.ofpattern("dd/MM/yy");
		 */

		accThuong.setSoTaiKhoan(soTaikhoan);
		accThuong.setHoTen(hoTen);
		accThuong.setSoCMT(soCMT);
		accThuong.setsDT(soDT);
		accThuong.setQueQuan(queQuan);
		accThuong.setSoDu(soDu);
		accThuong.setEmail(email);
		accThuong.setNgayThangNamSinh(ngayThangNamSinh);
		danhSachAccount.add(accThuong);
